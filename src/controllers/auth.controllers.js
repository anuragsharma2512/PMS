...
const login = asyncHandler(async (req,res)=>{
    ...
    if(!user){
        throw new ApiError(404, "User does not exist with the provided email")
    }
    ...
    res
        ...
        .json(
            new ApiResponse(
                200,
                {},
                "Mail was send successfully. Please check your email to verify your account."
            )
        )
...
const refreshAccessToken = asyncHandler(async(req,res)=>{
    const incomingRefreshToken = req.cookkies.refreshToken || req.body.refreshToken
    ...
    try {
        ...
        res
            .status(200)
            .cookkies("accessToken", accessToken, Option)
            .cookkies("refreshToken", newRefreshToken, Option)
            .json(
                new ApiResponse(
                    200,
                    {
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                {},
                "Password reset request sent successfully on your email"
            )
        )
})
